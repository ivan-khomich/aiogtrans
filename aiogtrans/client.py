...
class Translator:
    ...
    async def _build_rpc_request(self, text: str, dest: str, src: str):
        return json.dumps(
            [
                [
                    [
    ...
    async def _translate(self, text: str, dest: str, src: str):
        url = urls.TRANSLATE_RPC.format(host=await self._pick_service_url())
        data = {
    ...
    async def _translate_legacy(self, text, dest, src, override):
        ...
        params = await utils.build_params(
    ...
    async def translate_legacy(self, text, dest="en", src="auto", **kwargs):
        ...
        if isinstance(text, list):
            result = []
            for item in text:
                translated = await self.translate_legacy(
